...
class PersonEditCtrl {
	...
	public function validateSave() {
		...
		$this->form->telnum = getFromRequest('telnum', true, 'Błędne wywołanie aplikacji');
		...
		if (empty(trim($this->form->telnum))) {
		...
		if(!preg_match("/^\d{3}-\d{3}-\d{3}$/", trim($this->form->telnum))) {
			getMessages()->addError('Zły format numeru telefonu. Przykład: 111-222-333');
		}
	...
	public function action_personEdit(){
		...
		if ( $this->validateEdit() ){
			try {
				...
				$this->form->telnum = $record['telnum'];
	...
	public function action_personSave(){
		...
		if ($this->validateSave()) {
			...
			// 3b. Po zapisie przejdź na stronę listy osób
